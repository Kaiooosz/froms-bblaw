...
export default function WealthPlanningForm() {
    const [step, setStep] = useState(1);
    const [history, setHistory] = useState<number[]>([1]);
    const [isSubmitting, setIsSubmitting] = useState(false);
    const { toggleTheme, theme } = useTheme();

    const methods = useForm<WealthPlanningFormData>({
        defaultValues: {
            aceita_termos: false,
            composicao_patrimonio: [],
            quer_manter_controle: true,
            preocupacao_genros_noras: true
        }
    });
    ...
    const nextStep = async () => {
        ...
        if (step === 1) fieldsToValidate = ['nome_completo', 'whatsapp', 'email', 'cidade_estado', 'tipo_cliente'];
        if (step === 2) fieldsToValidate = ['valor_patrimonio_estimado', 'numero_herdeiros'];
    ...
    const onSubmit = async (data: WealthPlanningFormData) => {
        setIsSubmitting(true);
        try {
            const response = await fetch('/api/leads', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ ...data, formType: 'wealth-planning' }),
            });
    ...
    const stepTitles = ["Identificação", "Patrimônio", "Objetivos", "Concluído"];
            ...
            <main className="form-main-container">
                ...
                {step < 4 && (
                    <div className="sigilo-box mt-8">
                        <ShieldCheck size={24} />
                        <p>Planejamento sucessório exige sigilo absoluto. Seus dados estão criptografados.</p>
                    </div>
                )}
            </main>
        </div>
